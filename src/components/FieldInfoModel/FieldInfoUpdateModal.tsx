 ...
 * @constructor
 * */
export const FieldInfoUpdateModal:React.FC<PropsWithChildren<UpdateModalProps>> = (props) => {
  const {oldData,columns,modalVisible,onSubmit,onCancel} = props;

  return (
    <Modal destroyOnClose title="更新" open={modalVisible} onCancel={() => onCancel()} footer={null}>
      ...
      onSubmit={async(values) => {
        ...
        if(success) onSubmit?.();
      }} rowKey="id" type="form" form={{initialValues:oldData}} columns={columns}/>
    </Modal>
  )
}